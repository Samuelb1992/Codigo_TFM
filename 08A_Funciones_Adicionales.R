...
plot.compare <- function(x) {
  ggplot(x, aes(x = dataset, y = f1mean, col = method, linetype = method , group = method )) +
    geom_line(lwd = 1) +
    geom_point() +
    scale_linetype_manual(values = c("solid","dashed",rep("solid",5)) , name = "Método") +
    scale_color_manual(values = c("green","black",brewer.pal(5,"Set3")) , name = "Método") +
    labs(x = NULL , 
         y = "F1" ,
         title = "F1 Avg de mejor combinación de hiperparámetros") +
...
# logica de las funciones sin CV
# 1. hold out 70 - 30 para set de entrenamiento y prueba
# Hay que cambiar algunas cosas , train.data ; train.response ; test.data ; test.response
# Cambiar metricas en la parte de la matriz de confusion , largo del vector , etc


## Boruta sin CV -----------------------------------------------------------


BorutaRandomForest.Final <-  function(x, 
                                      y, 
                                      cutoff = 0.5, 
                                      folds = 1, 
                                      Rep = 1,
  ...
  # lista para guardar variables seleccionadas
  var.rep <- vector("list",Rep)
  # lista para guardar los modelos ajustados
  var.model <- vector("list",Rep)
  set.seed(seed)
  # Repeticiones
  for (k in 1:Rep) {
    folds.k <-  caret::createFolds(y = y, k = folds )
    ...
    var.list <- list()
    j = 1
    # Iteracion por cada fold -- Conjunto de entrenamiento es todo el fold y el test lo mismo
    for (i in folds.k) {
      trainIndex <- createDataPartition(y, p = 0.70, list = FALSE)
      train.data <- x[trainIndex,] # trainIndex seran los datos de entrenamiento
      test.data <- x[-trainIndex,] # -trainIndex seran los datos de test
      train.response <- y[trainIndex]
      test.response <- y[-trainIndex]
      # Quitar variables con varianza < 2.22e-16 ( criterio de funcion opls de librería ropls ocupado en TFM anterior)
      if ((isTRUE(varZero)) & (sum(diag(cov(train.data)) < 2.22e-16) > 0 )) {
        var.0 <- which(diag(cov(train.data)) < 2.22e-16)
        train.data <- train.data[, -var.0]
      ...
      # prediccion con ese punto de corte - Guardar prediccion de ese cut
      pred.list.hyp[[as.character(cutoff)]] <- ifelse(pred.rf > cutoff ,levels(y)[2]  , levels(y)[1])
    }
    # Sacamos de lista combinaciones de parámetros que no se ajustaron para todos los folds , es decir vector de predicciones tendran un largo menor
    pred.list.hyp.filter <- Filter(function(x) (length(x) == length(test.response)) & !any(is.na(x)) , pred.list.hyp)
    # Guardamos vector de predicciones
    pred.rep[[k]] <- pred.list.hyp.filter
    # Guardarmos el F1 Score
    if (length(pred.list.hyp.filter) > 0) {
      f1.rep[[k]] <- lapply(pred.list.hyp.filter,
                            function(r) caret::confusionMatrix(table(factor(r,levels(y)),test.response), positive = "d")$byClass["F1"])
    } else {
      f1.rep[[k]] <- NA
    }
    # Guardamos otras métricas
    meas.rep[[k]] <- lapply(pred.list.hyp.filter,function(r) caret::confusionMatrix(table(factor(r,levels(y)),test.response), positive = "d")$byClass)
    # Guardar variables seleccionadas en esa rep
    var.rep[[k]] <- var.list
    # Guardamos el ultimo modelo ajustado 
    var.model[[k]] <- rf.fold 
  }
  return(list("f1rep" = reshape2::melt(f1.rep),
              "pred"  = pred.rep,
...
VarSelRf.Final <-  function(x, 
  ...
  # lista para guardar variables seleccioanadas
  var.rep <- vector("list",Rep)
  # lista para guardar el modelo seleccionado
  var.model <- vector("list",Rep)
  set.seed(seed)
  # Repeticiones
  for (k in 1:Rep) {
    ...
    for (i in folds.k) {
      ...
      # Aplicamos varSelRF - como es una rep se cambia a parametros por default
      varSelRF.run <- tryCatch(varSelRF::varSelRF(train.data,
                                                  train.response,
                                                  ntree = 5000,
                                                  ntreeIterat = 2000,
    ...
    # Guardar variables seleccionadas en esa rep
    var.rep[[k]] <- var.list
    # Guardamos modelo seleccionado en esa rep
    var.model[[k]] <- varSelRF.run
  }
  return(list("f1rep" = reshape2::melt(f1.rep),
              "pred"  = pred.rep,
              "other" = meas.rep,
  ...
  for (k in 1:Rep) {
    ...
    for (i in folds.k) {
      ...
      while (!terminated){
        ...
        model.iter[[iter]] <- pls.fold
        # Calculo predicciones y F1 sobre test para eso necesito punto de corte
        prediction.raw <- predict(pls.fold, test.data, type = "raw")
        prediction.raw <- prediction.raw[,2,]
        pred.fold.vip[[paste(ncomp,vip.val,cutoff,iter,sep="-")]] <- ifelse(prediction.raw > cutoff ,levels(y)[2]  , levels(y)[1])
        # Guardo variables seleccionadas de iteracion actual
        var.iter[[iter]] <- names(train.data)
        # Calculo VIP
        VIP.value <- plsVarSel::VIP(pls.fold, pls.fold$ncomp)
    ...
    # Sacamos de lista combinaciones de parámetros que no se ajustaron para todos los folds , es decir vector de predicciones tendran un largo menor
    pred.list.vip.filter <- Filter(function(x) (length(x) == length(test.response)) & !any(is.na(x)) , pred.fold.vip)
    # Guardamos vector de predicciones
    pred.rep[[k]] <- pred.list.vip.filter
    # Guardarmos el F1 Score
    if (length(pred.list.vip.filter) > 0) {
    ...
    # Guardamos otras métricas
    meas.rep[[k]] <- lapply(pred.list.vip.filter,function(r) caret::confusionMatrix(table(factor(r,levels(y)),test.response), positive = "d")$byClass)
    ...
    # Guardar variables seleccionadas en esa rep  - se extraen variables de cada fold de la mejor iteracion
    var.rep[[k]] <- lapply(var.list , function(x) x[[iter.opt]])
    # Guardar modelo pls de la mejor iteracion
    var.model[[k]] <- model.iter[[iter.opt]]
    # Se elige mejor F1 score (mejor iteracion) para cada grupo de ncomp , vip y punto de corte y se guarda en esa repeticion
    f1.rep[[k]] <- f1.list %>%
      reshape2::melt() %>%
      tidyr::separate(L1, c("ncomp", "vip", "cut", "iter"), "-") %>%
...
Vip.Final <-  function(  x, 
  ...
  # lista para guardar variables seleccionadas
  var.rep <- vector("list",Rep)
  # lista para guardar los modelos seleccionados
  var.model <- vector("list",Rep)
  set.seed(seed)
  # Repeticiones
  for (k in 1:Rep) {
    ...
    for (i in folds.k) {
      ...
      prediction.raw <- prediction.raw[,2,]
      # prediccion con ese punto de corte - Guardar prediccion de ese cut - vip - comp
      pred.list.hyp[[paste(ncomp,vip.value,cutoff,sep = "-")]] <- ifelse(prediction.raw > cutoff ,levels(y)[2]  , levels(y)[1])
      # Guardar variables seleccionadas en ese fold
      var.list[[j]] <- names(which(vip.filter))
      j <- j+1
    }
